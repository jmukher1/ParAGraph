...
for GROWTH_PERCENT in 5
do
    GROWTH_RATE=$(echo "scale=2; $GROWTH_PERCENT/100" | bc)

    OUTPUT_FILE="./output/cpu-serial-profile-static-output-1p5p-${NUM_CYCLES}y-${GROWTH_PERCENT}p.edgelist"
    OUTPUT_LOG="./output/cpu-serial-profile-static-output-1p5p-${NUM_CYCLES}y-${GROWTH_PERCENT}p-${NUM_THREADS}t.log"
    OUTPUT_AUX="./output/cpu-serial-profile-static-output-1p5p-${NUM_CYCLES}y-${GROWTH_PERCENT}p.aux"
    OUT_FILE="./output/cpu-serial-profile-static-output-1p5p-${NUM_CYCLES}y-${GROWTH_PERCENT}p-${NUM_THREADS}t.out"
