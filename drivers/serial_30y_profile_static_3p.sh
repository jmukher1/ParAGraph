...
for GROWTH_PERCENT in 3
do
    GROWTH_RATE=$(echo "scale=2; $GROWTH_PERCENT/100" | bc)

    OUTPUT_FILE="./output/cpu-serial-profile-static-output-3p6p-${NUM_CYCLES}y-${GROWTH_PERCENT}p.edgelist"
    OUTPUT_LOG="./output/cpu-serial-profile-static-output-3p6p-${NUM_CYCLES}y-${GROWTH_PERCENT}p-${NUM_THREADS}t.log"
    OUTPUT_AUX="./output/cpu-serial-profile-static-output-3p6p-${NUM_CYCLES}y-${GROWTH_PERCENT}p.aux"
    OUT_FILE="./output/cpu-serial-profile-static-output-3p6p-${NUM_CYCLES}y-${GROWTH_PERCENT}p-${NUM_THREADS}t.out"
