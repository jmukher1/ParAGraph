...
int main(int argc, char* argv[]) {
    ...
    std::string network_model = main_program.get<std::string>("--network-model");
    double er_edge_probability = main_program.get<double>("--er-edge-probability");
    int er_edges_per_node = main_program.get<int>("--er-edges-per-node");
    printf("\nnetwork-model = %s", network_model.c_str());
    printf("\nInit ABM....");
    ABM* abm = new ABM(edgelist, nodelist, out_degree_bag, recency_probabilities, planted_nodes, alpha, fully_random_citations, preferential_weight, recency_weight, fitness_weight, growth_rate, num_cycles, same_year_proportion, output_file, auxiliary_information_file, log_file, num_processors, log_level, network_model, er_edge_probability, er_edges_per_node);
    printf("\nExec ABM....");
    execute(abm);
