...
void Graph::WriteGraph(std::string output_file) const {
    ...
    // ---- Build safe rev_arr ----
    int max_id = 0;
    for (const auto& [seq_id, _] : reverse_continuous_node_mapping)
        max_id = std::max(max_id, seq_id);

    std::vector<int> rev_arr(max_id + 1);
    for (const auto& [seq_id, orig_id] : reverse_continuous_node_mapping)
        rev_arr[seq_id] = orig_id;

    // ---- Snapshot keys ----
    std::vector<int> src_keys;
    src_keys.reserve(forward_adj_map.size());
    ...
    const int nt = std::min(omp_get_max_threads(), num_srcs);
    ...
    #pragma omp parallel num_threads(nt)
    {
        ...
        for (int i = begin; i < stop; ++i) {
            ...
            const int mapped_u = rev_arr[u];

            auto [p1, _] = std::to_chars(tmp, tmp + sizeof(tmp), mapped_u);
            const int u_len = p1 - tmp;

            const auto& nbrs = forward_adj_map.at(u);

            for (int v : nbrs) {
                buf.append(tmp, u_len);
                buf.push_back(',');

                auto [p2, _] = std::to_chars(tmp, tmp + sizeof(tmp), rev_arr[v]);
                buf.append(tmp, p2 - tmp);
                buf.push_back('\n');
            }
        }
    }
