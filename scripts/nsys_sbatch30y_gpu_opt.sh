...
mkdir -p output
mkdir -p errors
mkdir -p profile          # create profile dir alongside output/errors

OUTPUT="./output/"
ERRORS="./errors/"
PROFILE_DIR="./profile/"   # ← define it, absolute path
...
for GROWTH_PERCENT in $growth_percents; do
    ...
    OUTPUT_FILE="${OUTPUT}${NUM_CYCLES}y/gpu-opt-static-output-${MODEL}-${NUM_CYCLES}y-${GROWTH_PERCENT}p.edgelist"
    OUTPUT_LOG="${OUTPUT}${NUM_CYCLES}y/nsys-gpu-opt-static-${MODEL}-${NUM_CYCLES}y-${GROWTH_PERCENT}p.log"
    OUTPUT_AUX="${OUTPUT}${NUM_CYCLES}y/gpu-opt-static-output-${MODEL}-${NUM_CYCLES}y-${GROWTH_PERCENT}p.aux"
