...
mkdir -p output
mkdir -p profile
mkdir -p "./output/ablation/ncu-RTX4090"
...
# ── Thread utilization (active vs. predicated-off threads per warp instruction) ──
# Goal for both is 32 (full warp). Divergence between the two ratios below
# reveals divergence the compiler resolved via predication (selp) rather
# than branching, which the branch-target metrics further down would miss.
NCU_METRICS+="smsp__thread_inst_executed_per_inst_executed.ratio,"
NCU_METRICS+="smsp__thread_inst_executed_pred_on_per_inst_executed.ratio,"
# ── Warp/branch divergence ──────────────────────────────────────────────
# smsp__sass_average_branch_targets_threads_uniform.pct: % of branches
#   with a uniform target across all threads in the warp. Goal = 100%;
#   lower means more divergent branches. NOTE: some compiled kernels use
#   predication (selp) instead of an actual branch instruction, in which
#   case this metric can read 0 even with no real divergence — cross-check
#   against the raw sums below and the pred_on ratio above.
NCU_METRICS+="smsp__sass_average_branch_targets_threads_uniform.pct,"
# Raw branch-target counts to independently verify/recompute the % above:
#   divergence_pct = branch_targets_threads_divergent.sum / branch_targets.sum
NCU_METRICS+="smsp__sass_branch_targets.sum,"
NCU_METRICS+="smsp__sass_branch_targets_threads_divergent.sum,"
# Total executed branch instructions (context for the counts above)
NCU_METRICS+="smsp__sass_branch_targets_threads_uniform.sum,"
...
for GROWTH_PERCENT in $growth_percents; do
    ...
    for entry in "${CONFIGS[@]}"; do
        ...
        PROFILE_BASE="${PROFILE_DIR}/${LABEL}-ncu-H100-gpu-parallel-static-${MODEL}-${NUM_CYCLES}y-${GROWTH_PERCENT}p"
