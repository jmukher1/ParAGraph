...
for GROWTH_PERCENT in $growth_percents; do
    ...
    OUTPUT_FILE="${OUTPUT}${NUM_CYCLES}y/nsys-profile-H100-er-static-output-${NUM_CYCLES}y-${GROWTH_PERCENT}p.edgelist"
    OUTPUT_LOG="${OUTPUT}${NUM_CYCLES}y/nsys-profile-H100-er-static-${MODEL}-${NUM_CYCLES}y-${GROWTH_PERCENT}p.log"
    OUTPUT_AUX="${OUTPUT}${NUM_CYCLES}y/nsys-profile-H100-er-static-auxiliary-${NUM_CYCLES}y-${GROWTH_PERCENT}p.aux"
    OUT_FILE="${OUTPUT}${NUM_CYCLES}y/nsys-profile-H100-er-static-auxiliary-${NUM_CYCLES}y-${GROWTH_PERCENT}p.out" 
    NSYS_OUT="${PROFILE_DIR}nsys-profile-H100-${MODEL}-${NUM_CYCLES}y-${GROWTH_PERCENT}p"   # ← now uses absolute PROFILE_DIR
